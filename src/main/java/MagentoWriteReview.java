...
public class MagentoWriteReview {
	...
	public void writereview() throws Exception {
		...
		for (int i =7; i <= 49; i++) {
			...
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("nickname"))).sendKeys("Payal");
			wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("title"))).sendKeys("greattt");
